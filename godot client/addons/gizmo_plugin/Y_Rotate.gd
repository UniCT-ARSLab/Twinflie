...
func _process(delta):
	if get_parent().active :
		...
		if(dragged):
			...
			var face = get_parent().get_parent().global_transform.basis.y
			get_parent().get_parent().global_transform = original_transform
			if(rad2deg(face.dot(direction)) > 0):
				get_parent().get_parent().rotate_object_local(Vector3(0,1,0), start - angle)
			else:
				get_parent().get_parent().rotate_object_local(Vector3(0,1,0), angle - start)
